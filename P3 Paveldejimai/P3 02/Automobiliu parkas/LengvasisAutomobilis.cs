...
namespace Automobiliu_parkas
{
    class LengvasisAutomobilis : TransportoPriemone
    {
        ...
        {
            return String.Format("Valstybinis numeris: {0, -3} | Gamintojas: {1, -7} | Modelis: {2, -7} |" +
                "Pagaminimo metai ir menuo: {3, 4} | Technines galiojimo data: {4, 4} | Kuras: {5 , -9} | 100km/h: {6 , 4} |",
                CarNumber, CarName, CarModel, Year.ToShortDateString(),
                ExpirationDate.ToShortDateString(), Fuel, AverageConsumption);
        }
    }
}
